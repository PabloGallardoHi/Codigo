...
public class CameraStreamer extends Object {
    ...
    CameraStreamer(final int cameraIndex, final boolean useFlashLight, final int port,
                   final int previewSizeIndex, final int jpegQuality, final SurfaceHolder previewDisplay){
        super();

        if(previewDisplay == null){
            throw new IllegalArgumentException("previewDisplay no puede ser null");
        }
    ...
    private void startStreamingIfRunning() throws IOException{
        ...
        synchronized (mLock){
            ...
            camera.startPreview();
            mCamera = camera;
            // Para corregir el giro de 90 grados de la imagen mostrada
            // por pantalla
